...
state start begin
  ...
  ---      d.notice("Der Richtige Schlussel wurde gefunden, in wenigen Sekunden geht es in den nächsten Stock.")
    ...
    function get_settings()
        ...
        snow_dungeon_settings.need_level_group = 100                 --Benotigte Level fur Gruppenmitglieder
    ...
    when 20397.chat."Savaþý baþlat" begin --Eisiger Lowe
        ...
        if select("Evet","Hayýr, korkuyorum. ") == 1 then
            ...
            d.setf("level",1)
            d.notice("Bir sonraki kata çýkmak için Tüm yaratýklarý öldürmelisiniz.") 
            npc.purge()
    ...
    function _GetLastTime()
        ...
        if c >= o then
            if t >= 60 then
                d.notice("Yaklaþýk "..math.ceil(t / 60).." dakikanýz kaldý...")
            else
                d.notice("Yaklaþýk "..t .." saniyeniz kaldý...")
            end
            if t >= (60*10) then
    ...
    when CheckLevelAndCountKillOnMobs.server_timer begin
        if d.select(get_server_timer_arg()) then
            ...
            if level == 1 and mob_count <= 0 then
                d.notice("Tüm yaratýklarý maðlup ettin. Bir kaç dakika içerisinde bir sonraki kata ýþýnlanacaksýn.")
                d.notice("")
            ...
            elseif level == 6 and mob_count <= 0 then
                ...
                d.notice("Tüm canavarlarý yok ettiniz. "..mob_name(MobStonelvl6).."'i öldürün.")
                d.notice("")
                d.clear_regen()
                d.kill_all()
    ...
    when Jp_2.server_timer begin --Jump Level 2
        if d.select(get_server_timer_arg()) then
            ...
            d.notice("Yukarý çýkmak için "..item_name(v).."'i bulun.")
            d.notice("")
        end
    end
        ...
        if level == 2 then
            ...
            then
                if number(1,3) == 1 then --or d.count_monster() < 10 then
                    d.notice("Doðru anahtarý buldunuz! Bir kaç saniye sonra ýþýnlanacaksýnýz.")
                    ...
                    d.notice("Bu yanlýþ anahtar.")
                    item.remove()
                end
            else
                d.notice("Bunu sadece Þamanlar kullanabilir.")
                return
            end
        end
    end
    ...
    when Jp_3.server_timer begin --Jump Level 3
        if d.select(get_server_timer_arg()) then
            ...
            d.notice("Sonraki seviyeye ulaþmak için bütün yaratýklarý öldürün!")
        end
    end
    ...
    when Jp_5.server_timer begin --Jump Level 5
        if d.select(get_server_timer_arg()) then
            ...
            d.jump_all((cord[1] + pos[1]), (cord[2] + pos[2]))
            d.notice("Anahtar düþürerek "..mob_name(v).." isimli buz küpünden mühürü açýn. ")
            if set.IsNewga_me_re_vi_sion == true then
                d.notice("Anahtarý canavarlardan düþürebilirsiniz.")
            else
        ...
        if snow_dungeon.is_nemeres_dungeon() == true then
            ...
            if vnum == key_level_5 then
                        ...
                        if new > stone_level5_count then
                            d.notice("Son taþý açtýnýz, biraz sonra çýkacaksýnýz.")
                            npc.purge()
                            item.remove()
                            ...
                            d.notice(""..search..". Taþý açtýnýz, biraz daha ilerlemeniz gerekmekte. ".. stone_level5_count - search .." Adet taþýnýz kaldý.")
                            npc.purge()
                            item.remove()
                        end
    ...
    when Jp_6.server_timer begin --Jump Level 6
        if d.select(get_server_timer_arg()) then
            ...
            d.notice("Tüm canavarlarý öldürdünüz. Þimdi bütün canavarlarý yok edin ve "..mob_name(v).."'i maðlup edin.")
    ...
    when Jp_7.server_timer begin --Jump Level 7
        if d.select(get_server_timer_arg()) then
            ...
            d.jump_all((cord[1] + pos[1]), (cord[2] + pos[2]))
            d.notice("Metin taþarýný baþarýyla yok ettiniz. Þimdi "..mob_name(v).."'i maðlup edin!")
        end
    end
    ...
    when Jp_8.server_timer begin --Jump Level 8
        if d.select(get_server_timer_arg()) then
            ...
            d.jump_all((cord[1] + pos[1]), (cord[2] + pos[2]))
            d.notice("Yaratýklarý keserek "..item_name(v).." düþürün.")
            d.notice("Bu nesnenin dilinden sadece Þaman ve Ninjalar anlayabiliyor...")
        end
    end
        ...
        if level == 8 then
            ...
            then
                if number(1,3) == 1 then
                    d.notice("Doðru anahtarý buldunuz.Bir kaç saniye içinde ýþýnlanacaksýnýz.")
                    ...
                    d.notice("Bu anahtar yanlýþ.")
                    item.remove()
                end
            else
                d.notice("Bunu sadece Þaman ve Ninja karakterleri kullanabilir.")
                return
            end
        end
    end
    ...
    when Jp_9.server_timer begin --Jump Level 9
        if d.select(get_server_timer_arg()) then
            ...
            d.jump_all((cord[1] + pos[1]), (cord[2] + pos[2]))
            d.notice(""..mob_name(v).."'yi maðlup edin!")
            d.notice("Fakat ona sadece Þaman'ýn tecrübesi ve Ninja'nýn kuvveti karþý koyabilir.")
            d.notice("")
    ...
    when Jp_Boss.server_timer begin --Jump Level BossZone
        if d.select(get_server_timer_arg()) then
            ...
            d.jump_all((cord[1] + pos[1]), (cord[2] + pos[2]))
            d.notice(""..mob_name(v).."'yi dondurun ve maðlup edin!")
        end
    end
    ...
    when kill begin
        if snow_dungeon.is_nemeres_dungeon() == true then
            ...
            elseif level == 6 and mobvnum == stone_level_6 then
                d.notice(""..mob_name(stone_level_6).." maðlup edildi ve bir sonraki seviyeye geçiyorsunuz.")
                d.notice("")
            ...
            elseif level == 9 and mobvnum == stone_level_9 then
				d.notice(""..mob_name(stone_level_9).." maðlup edildi ve bir sonraki seviyeye geçiyorsunuz.")
            ...
            elseif level == 10 and mobvnum == boss_vnum then
                d.clear_regen()
                d.kill_all()
                d.notice("Tebrikler cesur savaþçýlar! "..mob_name(boss_vnum).."'yu maðlup ettiniz!.. Grup olarak "..wait_time.." saniye sonra ýþýnlanacaksýnýz.")
