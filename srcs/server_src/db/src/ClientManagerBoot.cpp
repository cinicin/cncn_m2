...
bool CClientManager::InitializeMobTable()
{
	...
	{
		fprintf(stderr, "mob_names.txt Dosyasi okunamadi\n");
		isNameFile = false;
	} else {
		nameData.Next();	//����row ����.
		while(nameData.Next()) {
	...
	{
		fprintf(stderr, "test dosyasi yok bu sekilde devam et.\n");
		isTestFile = false;
	}
	...
	if (isTestFile) {
		...
		while(test_data.Next()) {
			...
			{
				fprintf(stderr, "mob proto tablo ayarlamasi yarra yemis.\n");
			}
	...
	if(!data.Load("mob_proto.txt",'\t')) {
		fprintf(stderr, "mob_proto.txt dosyasi okunamadi\n");
		return false;
	}
	data.Next();					//���� row �Ѿ��
	//2. (!)[mob_table] �����ϱ�
	...
	if(!test_data.Load("mob_proto_test.txt",'\t'))
	{
		fprintf(stderr, "test dosyasi yok boyle devam et.\n");
		isTestFile = false;
	}
	if(isTestFile) {
		test_data.Next();	//���� �ο� �Ѿ��.
